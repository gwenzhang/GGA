...
from tools.data_converter.utils_gga import points_in_frustm_indices
...
def create_groundtruth_database(dataset_class_name,
    ...
    if dataset_class_name == 'KittiDataset_GGA':
    ...
    if database_save_path is None:
        database_save_path = osp.join(data_path, f'{info_prefix}_gt_database_GGA')
    if db_info_save_path is None:
        db_info_save_path = osp.join(data_path,
                                     f'{info_prefix}_dbinfos_train_GGA.pkl')
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        Trv2c = example['Trv2c']
        P2 = example['P2']
        lidar2img = example['lidar2img']
        GGA_gt_bboxes = annos['bboxes']
        GGA_boxes_img = annos['GGA_boxes_img']
        GGA_mask_depth = annos['GGA_mask_depth']
        GGA_mask2d = annos['GGA_mask2d']
        GGA_mask_valid = annos['GGA_mask_valid']
        GGA_mask_boundary = annos['GGA_mask_boundary']
        GGA_bdry_masks = annos['GGA_bdry_masks']
        GGA_in_box_points = annos['GGA_in_box_points']
        GGA_init_pseudo_label = annos['GGA_init_pseudo_label']
        GGA_num_points_in_box2d = annos['GGA_num_points_in_box2d']

        # filter invalid samples
        GGA_database_mask = GGA_mask2d & GGA_mask_valid
        names = names[GGA_database_mask]
        gt_boxes_3d = gt_boxes_3d[GGA_database_mask]
        difficulty = difficulty[GGA_database_mask]
        GGA_gt_bboxes = GGA_gt_bboxes[GGA_database_mask]
        GGA_boxes_img = GGA_boxes_img[GGA_database_mask]
        GGA_mask_depth = GGA_mask_depth[GGA_database_mask]
        GGA_mask2d = GGA_mask2d[GGA_database_mask]
        GGA_mask_valid = GGA_mask_valid[GGA_database_mask]
        GGA_mask_boundary = GGA_mask_boundary[GGA_database_mask]
        GGA_bdry_masks = GGA_bdry_masks[GGA_database_mask]
        GGA_in_box_points = [item for item, m in zip(GGA_in_box_points, GGA_database_mask) if m]
        GGA_init_pseudo_label = GGA_init_pseudo_label[GGA_database_mask]
        GGA_num_points_in_box2d = GGA_num_points_in_box2d[GGA_database_mask]

        num_obj = gt_boxes_3d.shape[0]
        assert GGA_boxes_img.shape[0] == num_obj, "The boxes are mismatched"
        assert GGA_gt_bboxes.shape[0] == num_obj, "The boxes are mismatched"

        """
        Notice: two ways to obtain copy-paste points:
        First: use the points which are projected in 2D boxes.
        Scecond: use the In-Box-Points. (Will try in the future.)
        """
        gt_point_indices = box_np_ops.points_in_rbbox(points, gt_boxes_3d) # Generated by 3D GT
        point_indices = []   ### change the points_in_rbbox to points_in_frustm
        for sign, box in zip(GGA_mask2d, GGA_boxes_img):
            if sign == True:
                point_indices.append(points_in_frustm_indices(points, rect, Trv2c, P2, box))
            else:
                point_indices.append(np.zeros((points.shape[0], 1)).astype(bool))
        
        if len(point_indices) != 0:
            point_indices = np.concatenate(point_indices, axis=-1)
        ...
        for i in range(num_obj):
            ...
            rel_filepath = osp.join(f'{info_prefix}_gt_database_GGA', filename)

            # save point clouds and image patches for each object
            GGA_points = points[point_indices[:, i]]
            # gt_points[:, :3] -= gt_boxes_3d[i, :3]  # GGA use the absolute coordinates
            ...
            if (used_classes is None) or names[i] in used_classes:
                db_info = {
                    ...
                    'num_points_in_gt': gt_point_indices[:, i].sum(),
                    'difficulty': difficulty[i],
                    'GGA_gt_box': GGA_gt_bboxes[i],
                    'GGA_box_img': GGA_boxes_img[i],
                    'GGA_mask_depth': GGA_mask_depth[i],
                    'GGA_mask2d': GGA_mask2d[i],
                    'GGA_mask_valid': GGA_mask_valid[i],
                    'GGA_mask_boundary': GGA_mask_boundary[i],
                    'GGA_bdry_mask': GGA_bdry_masks[i],
                    'GGA_in_box_points': GGA_in_box_points[i],
                    'GGA_init_pseudo_label': GGA_init_pseudo_label[i],
                    'GGA_num_points_in_box2d': GGA_num_points_in_box2d[i],
                    'GGA_lidar2img': lidar2img,
                }
